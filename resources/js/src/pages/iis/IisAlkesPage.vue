...
const columns = [
  ...
  {
    title: "Posisi / Lokasi dari IIS",
    key: "position_legacy",
    width: 250,
    ellipsis: true,
    sorter: (a, b) => (a.position_legacy || '').localeCompare(b.position_legacy || ''),
    sortDirections,
  },
  {
    title: "Kategori IIS",
  ...
  },
  {
    ...
    key: "building_id",
    width: 140,
    sorter: (a, b) => (a.building_id || '').localeCompare(b.building_id || ''),
    sortDirections,
  },
  {
    title: "Lantai",
...
export default {
  ...
  computed: {
    ...
    canPrintUnlimited() {
      return this.can('iis.alkes.print.unlimited');
    },
    ...
    filteredBuildingsBulk() {
      if (!this.bulkForm.branch_id) return [];

      return this.constant.BUILDINGS.filter(
        b => b.branch_id === this.bulkForm.branch_id
      );
    },
    ...
    floorListBulk() {
      const building = this.constant.BUILDINGS.find(
        b => b.id === this.bulkForm.building_id
      );
  ...
  watch: {
    ...
    'bulkForm.branch_id'(val, oldVal) {
      if (val !== oldVal) {
        this.bulkForm.building_id = null
        this.bulkForm.floor = null
        this.bulkForm.unit_id = null
        this.bulkForm.room_id = null
      }
    },
    'bulkForm.building_id'(val, oldVal) {
      if (val !== oldVal) {
        this.bulkForm.floor = null
        this.bulkForm.room_id = null
        this.roomOptionsBulk = []
      }
    },
    'bulkForm.floor'(val, oldVal) {
      if (val !== oldVal) {
        this.bulkForm.room_id = null
  ...
  methods: {
    ...
    async writeData() {
      ...
      if (response && response.data) {
        if (!form.id) {
          vm.openNotification('Berhasil menambahkan data ALKES', 'success');
        }
        else {
          vm.openNotification('Berhasil mengubah data ALKES', 'success');
        }
        vm.readData();
        vm.showModal = false;
      }
    ...
    async fetchRooms(param = "", buildingId = null, floor = null, mode = "form") {

      const building = buildingId ?? this.form.building_id
      const floorVal = floor ?? this.form.floor

      if (!building || !floorVal) {
        if (mode === "bulk") this.roomOptionsBulk = []
        else this.roomOptions = []
        return
      }
      ...
      try {
        ...
        url += `building_id=${building}`
        url += `&floor=${encodeURIComponent(floorVal)}`
    ...
    onSearchRoomBulk: debounce(function (val) {
      this.fetchRooms(val, this.bulkForm.building_id, this.bulkForm.floor, "bulk")
    }, 500),
    ...
    handleActionClick(key) {
      ...
      if (['change_condition', 'change_pj', 'change_location', 'toggle_status'].includes(key)) {
        this.actionType = key;
        this.resetBulkForm();
    ...
    async handleBulkSubmit() {
      ...
      if (response && response.data) {
        ...
        switch (vm.actionType) {
          case 'change_condition':
            vm.openNotification('Berhasil mengubah kondisi ALKES', 'success');
            break;
          ...
          case 'change_location':
            vm.openNotification('Berhasil mengubah data lokasi', 'success');
            break;

          case 'toggle_status':
            vm.openNotification('Berhasil mengubah status keaktifan ALKES', 'success');
            break;
...
<template>
  ...
  <a-card>
      ...
      <a-col :xs="24" :sm="24" :md="18" class="flex justify-end">
        <a-row class="flex flex-wrap gap-2 justify-start md:justify-end w-full md:w-auto">
          ...
          <a-col class="w-full md:w-auto">
            <a-select v-model:value="filter.branch_id" placeholder="--Pilih Cabang--" show-search allow-clear
              option-label-prop="label" option-filter-prop="label" class="w-full lg:w-58" @change="readData">
    ...
    <a-table :scroll="{ x: 800 }" :columns="columns" :row-key="(obj) => obj.qr_code_no" :pagination="_pagination"
      ...
      <template #bodyCell="{ index, column, record }">
        ...
        <template v-if="column.key === 'item_iis'">
          <a-tag v-if="record.item_no_legacy && record.b_item_iis" color="blue">
            <span class="text-sm">{{ record.b_item_iis?.item_no }} - {{ record.b_item_iis?.name }}</span>
          </a-tag>
          <a-tag v-else color="red">
            <span>{{ record.item_no_legacy }}</span>
          </a-tag>
        </template>
        ...
        <template v-if="column.key === 'position'">
         ...
         <template v-if="column.key === 'position_legacy'">
          <a-tag color="red">
            <span class="text-sm">
              <span>{{ record.position_legacy }}</span>
            </span>
          </a-tag>
        </template>
  ...
  <!-- Modal Preview PDF -->
  <a-modal v-model:open="showPDFModal" title="Pratinjau Label ALKES IIS" width="900px" :mask-closable="false">
  ...
  <a-modal v-model:open="showPrintConfirm" title="Konfirmasi Cetak Label ALKES" :mask-closable="false" width="520px">

    <!-- ALERT -->
    <a-alert type="warning" show-icon class="mb-4" :message="printMode === 'single'
      ? 'Perhatian!'
      : 'Perhatian Cetak Massal!'" :description="printMode === 'single'
        ? 'Label ALKES memiliki batas maksimal cetak.'
  ...
  <!-- Modal Action -->
  <a-modal v-model:open="showActionModal" title="Pilih Aksi" :footer="null" width="900px">
    <div class="grid lg:grid-cols-3 gap-4 lg:mt-4 mb-4">
  ...
  <a-modal v-model:open="showBulkActionModal" :title="modalTitle" width="600px" @ok="handleBulkSubmit"
    ...
    <!-- === CHANGE Location === -->
    <template v-else-if="actionType === 'change_location'">
      <a-form layout="vertical">
        <!-- CABANG -->
        <a-form-item label="Cabang" data-column="branch_id" :rules="[{ required: true }]">
          <a-select v-model:value="bulkForm.branch_id" placeholder="--Pilih Cabang--" option-label-prop="label"
        ...
        <!-- GEDUNG -->
        <a-form-item label="Gedung" required>
          <a-select v-model:value="bulkForm.building_id" placeholder="-- Pilih Gedung --" allow-clear show-search
            option-label-prop="label">
            <a-select-option v-for="b in filteredBuildingsBulk" :key="b.id" :value="b.id" :label="b.name">
              {{ b.name }}
            </a-select-option>
          </a-select>
        </a-form-item>
        ...
        <!-- LANTAI -->
        <a-form-item label="Lantai" required>
          <a-select v-model:value="bulkForm.floor" placeholder="-- Pilih Lantai --" allow-clear show-search
            :disabled="!bulkForm.building_id" option-label-prop="label">
            <a-select-option v-for="f in floorListBulk" :key="f.value" :value="f.value" :label="f.label">
              {{ f.label }}
            </a-select-option>
          </a-select>
        </a-form-item>
        ...
        <a-form-item label="Unit">
          <a-select v-model:value="bulkForm.unit_id" show-search allow-clear class="w-full" placeholder="--Pilih Unit--"
        ...
        <!-- RUANGAN -->
        <a-form-item label="Ruangan">
          <a-select v-model:value="bulkForm.room_id" placeholder="--Pilih Ruangan--" show-search allow-clear
            :disabled="!bulkForm.building_id || !bulkForm.floor" :filter-option="false" @search="onSearchRoomBulk">
            <a-select-option v-for="r in roomOptionsBulk" :key="r.id" :value="r.id" :label="r.name">
              <div class="flex flex-col">
  ...
  <a-modal v-model:open="showModal" :title="form.id ? 'Ubah Data ALKES' : 'Tambah Data ALKES'" width="1000px"
    ...
    <a-form ref="formRef" layout="vertical">
      ...
      <div class="mb-4 p-4 rounded-xl border bg-blue-50">
          ...
          <a-select v-model:value="form.item_code" placeholder="--Pilih Item--" show-search :filter-option="false"
            ...
            <a-select-option v-for="item in itemOptions" :key="item.code" :value="item.code" :label="item.name">
              <div class="flex items-center gap-2">
                ...
                <a-tag color="blue">Kode IIS: {{ item.code_legacy }}</a-tag>
        ...
        <a-form-item label="Kategori" data-column="category_name" :rules="[{ required: true }]">
          <a-select v-model:value="form.category_name" placeholder="--Pilih Kategori--" show-search allow-clear
            option-filter-prop="label" class="w-full lg:w-96">
            <a-select-option v-for="cat in constant.CATEGORIES" :key="cat.id" :value="cat.category_name"
      ...
      <div class="mb-4 p-4 rounded-xl border bg-blue-50">
        ...
        <!-- ================= DISTRIBUSI ================= -->
        <div v-if="form.location_type === 'distributed'" class="grid grid-cols-2 lg:gap-4">
      ...
      <!-- ================= CATATAN ================= -->
      <div class="mb-4 p-4 rounded-xl border bg-blue-50">
        <h3 class="font-semibold text-slate-700 mb-3">Catatan ALKES</h3>
