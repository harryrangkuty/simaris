    ...
    methods: {
        ...
        async editData(m) {
            ...
            try {
                ...
                vm.form.from_location = firstItem?.from_location ?? {
                    branch_id: null,
                    warehouse_id: null,
                };
        ...
        async writeData() {
            ...
            if (response?.data?.success) {
                ...
                if (!vm.form.id) {
                    vm.openNotification('Serah terima distribusi berhasil dibuat ...', 'success');
                }
                else {
                    vm.openNotification('Serah terima distribusi berhasil diupdate', 'success');
                }
...
<template>
        ...
        <a-row class="flex flex-wrap items-start justify-between mb-4 pb-4 border-b-2 gap-y-4">
            ...
            <a-col :xs="24" :sm="24" :md="18" class="flex justify-end">
                <a-row class="flex flex-wrap gap-2 justify-start md:justify-end w-full md:w-auto">
                    ...
                    <!-- PJ -->
                    <a-col v-if="can('iis.distribution.create')" class="w-full md:w-auto">
                ...
                <template v-if="column.key === 'action'">
                    <a-button-group class="flex justify-center">
                        ...
                        <a-tooltip :title="record.status === 'draft' ? 'Submit' : 'Verifikasi'" v-if="
                            (can('iis.distribution.submit')
                                && record.status === 'draft'
                                && record.operator_id == user.id)
                            ||
                            (can('iis.distribution.verify')
                        ...
                        <!-- Edit -->
                        <a-tooltip title="Edit Data" v-if="can('iis.distribution.update') && record.status === 'draft'">
                        ...
                        <a-tooltip title="Hapus Data"
                            v-if="can('iis.distribution.delete') && record.status === 'draft'">
    ...
    <!-- ================= MODAL NEW / UPDATE ================= -->
    <a-modal v-model:open="showModal" :title="form.id ? 'Ubah Data Distribusi' : 'Tambah Data Distribusi'"
        width="1200px" @ok="writeData" :mask-closable="false" :destroy-on-close="true" :style="{ top: '30px' }">
        <a-form layout="vertical">
            <a-row :gutter="16">

                <a-col :xs="24" :md="12" :lg="8" v-if="!form.id">
                ...
                <a-col :xs="24" :md="12" :lg="8" v-if="!form.id">
                    <a-form-item label="Distribusi ke" required>
                ...
                <a-col :xs="24" :md="12" :lg="8" v-if="!form.id">
                    <a-form-item label="Catatan Distribusi">
    ...
    <a-modal v-model:open="confirmModalVisible" :title="confirmAction === 'submit'
        ? 'Konfirmasi Submit Distribusi'
        : 'Konfirmasi Verifikasi Distribusi'" width="820px" :mask-closable="false">
        <div class="bg-slate-50 rounded-2xl p-5 space-y-5">
                ...
                <div class="flex-1">
                    ...
                    <div class="text-lg font-semibold text-slate-800">
                        {{ confirmAction === 'submit'
                            ? 'Konfirmasi Submit Distribusi'
                            : 'Konfirmasi Verifikasi Distribusi' }}
                    </div>
    ...
    <!-- Modal Preview PDF -->
    <a-modal v-model:open="showPDFModal" title="Berkas Digital Serah Terima Distribusi Barang" width="900px"
        :mask-closable="false">
        <div class="relative">
    ...
    <a-modal v-model:open="showRejectionModal" title="Alasan Penolakan Distribusi" width="500px" :mask-closable="false">

        <!-- ALERT -->
        <a-alert type="error" show-icon class="mb-4">
            <template #message>
                Serah terima distribusi ditolak
            </template>
