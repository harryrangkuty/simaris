...
const menuItems = [
    ...
    { key: "detail", label: "Detail Inventaris", icon: "fluent-color:apps-list-detail-20" },
    { key: "location", label: "Lokasi Inventaris", icon: "streamline-flex-color:map-location-flat" },
    ...
    computed: {
        breadcrumbItems() {
            return [
                ...
                { label: 'Daftar Inventaris IIS', link: '/iis/inventories-list', icon: 'line-md:folder-multiple-filled' },
            ...
            <a-col :xs="24" :sm="24" :lg="19">
                <a-card class="shadow-md rounded-b-2xl rounded-t-none h-full">
                    <transition name="fade" mode="out-in">
                        <div :key="activeMenu">
                            <!-- MENU PJ -->
                            <InventoryPJ v-if="activeMenu === 'pj_info'" :model-data="models" @save="handleSave" />

                            <!-- MENU Detail -->
                            <InventoryDetail v-if="activeMenu === 'detail'" :model-data="models" @save="handleSave" />

                            <!-- MENU LOKASI -->
                            <InventoryLocation v-if="activeMenu === 'location'" :model-data="models"
                                :constant="constant" @save="handleSaveLocation" />

                            <!-- MENU KONDISI -->
                            <InventoryCondition v-if="activeMenu === 'condition'" :model-data="models"
                                @save="handleSave" />

                            <!-- MENU MAINTENANCE -->
                            <InventoryMaintenance v-if="activeMenu === 'maintenance'" :model-data="models" />

                            <!-- MENU QR CODE -->
                            <InventoryQr v-if="activeMenu === 'qr_code_no'" :model-data="models" />
