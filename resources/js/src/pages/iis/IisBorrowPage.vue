...
export default {
    ...
    data() {
        return {
            ...
            form: {
                id: null,
                to_type: null,   // user | unit
                to_pj_id: null,
                asset_type: null,
                notes: null,
                borrowed_at: null,
                borrowed_due_at: null,
                from_location: {
                    branch_id: null,
                    warehouse_id: null
                },
    ...
    methods: {
        ...
        async fetchAssets() {
            const vm = this;
            if (!vm.form.from_location.warehouse_id || !vm.form.asset_type) return;
            ...
            const params = {
                ...
                warehouse_id: vm.form.from_location.warehouse_id,
                movement_id: vm.form.id
            };

            const response = await vm.axios
                .get("/lookups/iis-available-distribution-assets", { params })
        ...
        async writeData() {
            ...
            if (response?.data?.success) {
                ...
                if (!vm.form.id) {
                    vm.openNotification('Serah terima pinjaman berhasil dibuat ...', 'success');
                }
                else {
                    vm.openNotification('Serah terima pinjaman berhasil diupdate', 'success');
                }
...
<template>
        ...
        <a-row class="flex flex-wrap items-start justify-between mb-4 pb-4 border-b-2 gap-y-4">
            ...
            <a-col :xs="24" :sm="24" :md="18" class="flex justify-end">
                <a-row class="flex flex-wrap gap-2 justify-start md:justify-end w-full md:w-auto">
                    ...
                    <!-- PJ -->
                    <a-col v-if="can('iis.borrow.create')" class="w-full md:w-auto">
                        <a-select v-model:value="filter.to_pj_id" placeholder="--Pilih PJ--" show-search allow-clear
                ...
                <template v-if="column.key === 'action'">
                    <a-button-group class="flex justify-center">
                        ...
                        <a-tooltip :title="record.status === 'draft' ? 'Submit' : 'Verifikasi'" v-if="
                            (can('iis.borrow.submit')
                                && record.status === 'draft'
                                && record.operator_id == user.id)
                            ||
                            (can('iis.borrow.verify')
                        ...
                        <!-- Edit -->
                        <a-tooltip title="Edit Data" v-if="can('iis.borrow.update') && record.status === 'draft'">
                        ...
                        <!-- Hapus -->
                        <a-tooltip title="Hapus Data" v-if="can('iis.borrow.delete') && record.status === 'draft'">
    ...
    <!-- ================= MODAL NEW / UPDATE ================= -->
    <a-modal v-model:open="showModal" :title="form.id ? 'Ubah Data Peminjaman' : 'Tambah Data Peminjaman'"
        ...
        <a-form layout="vertical">
            <a-row :gutter="16">
                ...
                <a-col :xs="24" :md="12" :lg="8">
                    <a-form-item label="Peminjaman ke" data-column="to_type" required>
                        <a-select v-model:value="form.to_type" placeholder="--Pilih Tujuan--" allow-clear>
                            <a-select-option value="user">Perorangan (PJ)</a-select-option>
                            <a-select-option value="unit">Unit</a-select-option>
                        </a-select>
                    </a-form-item>
                </a-col>

                <a-col :xs="24" :md="12" :lg="8">
                    <a-form-item label="Catatan Peminjaman">
                ...
                <div class="px-2 w-full">
                    <div class="mb-2 py-2 px-4 rounded-xl border bg-blue-50">
                        <h3 class="font-semibold text-slate-700 mb-3">Tenggat</h3>
                        <div class="grid lg:grid-cols-2 lg:gap-4">

                            <a-form-item label="Tanggal Pinjam" data-column="borrowed_at" required>
                                <a-date-picker v-model:value="form.borrowed_at" style="width: 100%"
                                    placeholder="Pilih tanggal pinjam" :format="'DD/MM/YYYY'" />
                            </a-form-item>

                            <a-form-item label="Tanggal Jatuh Tempo" data-column="borrowed_due_at" required>
                                <a-date-picker v-model:value="form.borrowed_due_at" style="width: 100%"
                                    placeholder="Pilih tanggal kembali" :format="'DD/MM/YYYY'" />
                            </a-form-item>
                ...
                <!-- ================= LOKASI ASAL ================= -->
                <div class="px-2 w-full">
                    <div class="mb-2 py-2 px-4 rounded-xl border bg-purple-50">
                        <h3 class="font-semibold text-slate-700 mb-3">Lokasi Asal</h3>
                        <div class="grid lg:grid-cols-2 lg:gap-4">

                            <a-form-item label="Cabang Asal" data-column="from_location.branch_id"
                                :rules="[{ required: true }]">
                                <a-select v-model:value="form.from_location.branch_id"
                                    placeholder="--Pilih Cabang Asal--" option-label-prop="label"
                                    option-filter-prop="label" show-search allow-clear>
                            ...
                            <a-form-item label="Pilih Gudang Asal" data-column="to_location.warehouse_id"
                                :rules="[{ required: true }]">
                                <a-select v-model:value="form.from_location.warehouse_id"
                                    placeholder="--Pilih Gudang Asal--" allow-clear show-search
                                    ...
                                    <a-select-option v-for="w in filteredWarehouses" :key="w.id" :value="w.id"
                                        ...
                                        <div class="flex items-start gap-3">
                                            ...
                                            <div class="flex flex-col leading-tight">
                                                <span class="font-medium text-slate-800">{{ w.code }} - {{ w.name }} -
                                                    {{
                                                        w.branch?.name || '-'
                                                    }}</span>
                                                <span class="text-xs text-gray-500">
                                                    PJ : {{ w.person_in_charge?.identifier || '-' }} - {{
                                                        w.person_in_charge?.name || '-' }}<span
                                                        v-if="w.person_in_charge?.position"> • {{
                                                            w.person_in_charge.position }}</span>
                                                </span>
                                            </div>
                                        </div>
                                    </a-select-option>
                        ...
                        <div class="grid lg:grid-cols-3 lg:gap-4">
                            ...
                            <a-form-item label="Penanggung Jawab (PJ) Barang" data-column="to_pj_id"
                                ...
                                <a-select v-model:value="form.to_pj_id" placeholder="--Pilih PJ--" show-search
                                    ...
                                    option-filter-prop="label" @search="onSearchUser">
                ...
                <a-col :span="12">
                    <div class="relative bg-white rounded-xl p-4 shadow-sm border border-blue-100 h-full">
                        ...
                        <div class="space-y-3 text-sm pl-2">
                            ...
                            <div class="flex items-start gap-3">
                                <Icon icon="mdi:source-branch" class="text-blue-400 mt-1" />
                                <div>
                            ...
                            <div class="flex items-start gap-3">
                                <Icon icon="mdi:warehouse-outline" class="text-blue-400 mt-1" />
                                <div>
                                    <div class="text-xs text-slate-500">Gudang</div>
                                    <div class="font-medium">
                                        {{ selectedMovement?.from_location?.warehouse_name ?? '-' }}
                                    </div>
                                </div>
                            </div>
    ...
    <a-modal v-model:open="confirmModalVisible" :title="confirmAction === 'submit'
        ...
        : 'Konfirmasi Verifikasi Peminjaman'" width="820px" :mask-closable="false">
        <div class="bg-slate-50 rounded-2xl p-5 space-y-5">
                ...
                <div class="flex-1">
                    ...
                    <div class="text-lg font-semibold text-slate-800">
                        {{ confirmAction === 'submit'
                            ? 'Konfirmasi Submit Peminjaman'
                            : 'Konfirmasi Verifikasi Peminjaman' }}
                    </div>
    ...
    <!-- Modal Preview PDF -->
    <a-modal v-model:open="showPDFModal" title="Berkas Digital Serah Terima Peminjaman Barang" width="900px"
        :mask-closable="false">
    ...
    <a-modal v-model:open="showRejectionModal" title="Alasan Penolakan Peminjaman" width="500px" :mask-closable="false">

        <!-- ALERT -->
        <a-alert type="error" show-icon class="mb-4">
            <template #message>
                Serah terima pinjaman ditolak
            </template>
