...
const columns = [
    ...
    },
    {
        ...
        key: "verified_by",
        width: 240,
        sorter: (a, b) => a.verified_by - b.verified_by,
        sortDirections,
    },
    {
        title: "Total Item",
        ...
        <a-row class="flex flex-wrap items-start justify-between mb-4 pb-4 border-b-2 gap-y-4">
            ...
            <a-col :xs="24" :sm="24" :md="18" class="flex justify-end">
                <a-row class="flex flex-wrap gap-2 justify-start md:justify-end w-full md:w-auto">
                    ...
                    <a-col v-if="can('iis.return.create')" class="w-full md:w-auto">
        ...
        <a-table :scroll="{ x: 800 }" :columns="columns" :data-source="models" :row-key="obj => obj.id"
            ...
            <template #bodyCell="{ index, column, record }">
                ...
                <template v-if="column.key === 'action'">
                    <a-button-group class="flex justify-center">
                        ...
                        <a-tooltip :title="record.status === 'draft' ? 'Submit' : 'Verifikasi'" v-if="
                            ...
                            // VERIFY
                            (can('iis.return.verify') && record.status === 'submitted')
                        ">
                            <div class="px-2">
                        ...
                        <a-tooltip title="Tolak Serah Terima" v-if="
                            can('iis.return.reject')
                            && record.status === 'submitted'
                            && can('iis.return.verify')
                        ">
                            <div class="px-2">
                        ...
                        <!-- Edit -->
                        <a-tooltip title="Edit Data" v-if="can('iis.return.update') && record.status === 'draft'">
                        ...
                        <!-- Hapus -->
                        <a-tooltip title="Hapus Data" v-if="can('iis.return.delete') && record.status === 'draft'">
                ...
                <template v-if="column.key === 'verified_by'">
                    <a-tag color="red">
                        {{ record.verifier?.identifier }} - {{ record.verifier?.name }}
                    </a-tag>
                </template>
                        ...
                        <div class="space-y-3 text-sm pl-2">
                            ...
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <div class="font-medium">
                                        {{ selectedMovement?.to_location?.warehouse_name ?? '-' }}
                                    </div>
                                </div>
                            </div>
