...
export default {
  ...
  data() {
    return {
      ...
      showPDFPrevModal: false, // modal PDF viewer
      pdfPrevUrl: null,
      pdfLoading: false, // HTML dari backend
    };
  },
  ...
  methods: {
    ...
    exportToExcel() {
      ...
      const filteredParams = Object.fromEntries(
        Object.entries(vm.filter).filter(([key, value]) => value != null)
      );

      const params = new URLSearchParams({ ...filteredParams, req: 'excel' });
      const url = `${vm.readRoute}?${params.toString()}`;
    ...
    exportToPdf() {
      ...
      vm.$nextTick(() => {
        ...
        filteredParams.req = "pdf";
        const params = new URLSearchParams(filteredParams);
        vm.pdfPrevUrl = `${vm.readRoute}?${params.toString()}`;
      });
    },
...
<template>
        ...
        <a-row class="flex flex-wrap gap-2 justify-start md:justify-end w-full md:w-auto">
          ...
          <a-col class="w-full md:w-auto">
            <a-button type="primary"
              class="bg-gradient-to-r from-green-400 via-green-500 to-green-600 hover:from-yellow-400 hover:via-yellow-600 hover:to-purple-500 flex items-center justify-center text-white font-medium border-0 shadow-md transition-all duration-300 w-full"
              @click="exportToExcel">
              <Icon icon="vscode-icons:file-type-excel2" class="mr-1" />
              Export Excel
            </a-button>
          </a-col>
          ...
          <a-col class="w-full md:w-auto">
            <a-button type="primary"
              ...
              @click="exportToPdf">
              <Icon icon="vscode-icons:file-type-pdf2" class="mr-1" />
              Export PDF
            </a-button>
          </a-col>
  ...
  <!-- Modal Preview PDF -->
  <a-modal v-model:open="showPDFPrevModal" title="Preview PDF" width="900px">
    <div v-if="pdfLoading" class="absolute inset-0 flex items-center justify-center bg-white/70 z-10">
      <a-spin size="large" tip="Loading PDF..." />
    </div>
    <iframe v-if="pdfPrevUrl" :src="pdfPrevUrl" style="width:100%;height:600px;" frameborder="0"
      @load="pdfLoading = false" />
    <template #footer>
      <a-button type="primary" @click="showPDFPrevModal = false">Tutup</a-button>
    </template>
  </a-modal>
</template>