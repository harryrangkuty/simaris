...
class IisACController extends AuthController
{
        ...
        if ($request->req === 'open') {
            ...
            if ($request->has('xid')) {
                if ($request->user()->hasRole('superadmin')) {
                    $data = IisInventory::where('id', $request->xid)
                        ->first();
                } else {
                    abort(403);
                }
                ...
                $data->is_handover_active = !!$handover;
            ...
            $title = 'Data AC '.$data->qr_code_no;
            $vue = "<iis-ac-detail-page :title='".json_encode($title)."' :constant='".json_encode($constant)."' :parent='".json_encode($data)."' />";
        } else {
            $constant = [
            ...
            $title = 'Daftar AC IIS';
            $vue = "<iis-ac-page :title='".json_encode($title)."' :constant='".json_encode($constant)."' />";
        }
    ...
    public function read(Request $request)
    {
        if ($request->req == 'table') {
            ...
            $data = IisInventory::with(['bUser', 'lastPrintBy', 'unit', 'room', 'item', 'building'])
                ->where(function ($q) {
                    $q->whereIn('category_name', [
                        ...
                        ->orWhere(function ($qq) {
                            $qq->where('description', 'like', 'AC %')
                                ->orWhere('description', 'like', '% AC %')
                                ->orWhere('description', 'like', '% AC')
                                ->orWhere('description', '=', 'AC');
                        });
                })
                ...
                ->where(function ($q) use ($request) {
                    if ($request->unit) {
                        $q->whereHas('unit', function ($uq) use ($request) {
                            ...
                            ->orWhere('unit_legacy', 'like', "%{$request->unit}%");
                    }
                })
                ->where(function ($q) use ($request) {
                    if ($request->user) {
            ...
            $data = IisInventory::with([
                ...
                'room',
                'building',
                'creator:id,identifier,name',
                'updater:id,identifier,name',
            ])->findOrFail($request->id);

            // ============================
            // FLAG HANDOVER ACTIVE
            // ============================
            $data->is_handover_active = IisQrCodeHandover::whereIn('status', ['draft', 'submitted', 'approved'])
                ->whereHas('items', function ($q) use ($data) {
                    $q->where('qr_code_no', $data->qr_code_no);
                })
                ->exists();
            ...
            $data = IisInventory::findOrFail($request->id);

            // $url = route('iis.inventory_profile.access', [
            //     'code' => $data->access_code,
            // ]);

            // Barcode dengan logo ditengah
            // $qrBase64 = $this->makeQrBase64WithLogo(
            //     $url,
            //     public_path('images/logo-rsbt.png')
            // );
            ...
            $views = [
                'standard' => 'print.iis.iis_inventaris_label_single_standard',
                'mini' => 'print.iis.iis_inventaris_label_single_mini',
            ];
            ...
            if ($data->print_count >= 2 && ! $user->can('iis.inventory.print.unlimited')) {
                return response()->json([
                    'success' => false,
            ...
            $data = IisInventory::whereIn('qr_code_no', $ids)->get();
            ...
            $items = $data->map(function ($item) {
                ...
                return [
                    ...
                    // 'url' => $url,
                    // Barcode dengan logo ditengah
                    // 'qrBase64' => $this->makeQrBase64WithLogo($url, $logoPath),
                    // Barcode tanpa logo
            ...
            // ===============================
            $views = [
                'standard' => 'print.iis.iis_inventaris_label_multi_standard',
                'mini' => 'print.iis.iis_inventaris_label_multi_mini',
            ];
            ...
            $items = IisInventory::whereIn('qr_code_no', $ids)->get();
            ...
            foreach ($items as $item) {
                if ($item->print_count >= 2 && ! $user->can('iis.inventory.print.unlimited')) {
                    return response()->json([
                        'success' => false,
    ...
    public function write(Request $request)
    {
        if ($request->req === 'write') {
            ...
            // ================= FIND DATA =================
            $data = IisInventory::find($request->id);
            ...
            if (! $data) {
                ...
                DB::transaction(function () use ($request, &$data) {
                    ...
                    $data->asset_number = $sequence->last_number;
            ...
            // VALIDASI KHUSUS PJ
            // ============================
            if ($request->field === 'pj_nik') {

                // Sudah handover
                if ($data->is_handed_over) {
                    throw ValidationException::withMessages([
                        'value' => "Asset barcode {$data->qr_code_no} sudah diserahterimakan, tidak bisa ubah PJ",
                    ]);
                }

                // Ada proses handover aktif
                $active = IisQrCodeHandover::whereIn('status', ['draft', 'submitted', 'approved'])
                    ->whereHas('items', function ($q) use ($data) {
                        $q->where('qr_code_no', $data->qr_code_no);
                    })
                    ->exists();
                ...
                if ($active) {
                    throw ValidationException::withMessages([
                        'value' => "Asset barcode {$data->qr_code_no} sedang dalam proses serah terima",
                    ]);
                }
            }
            ...
            $request->validate([
                ...
                'action' => 'required|string|in:change_condition,change_pj,toggle_status',
            ]);

            $query = IisInventory::whereIn('qr_code_no', $request->ids);
