...
class IisBorrowController extends AuthController
{
    public function __invoke(Request $request)
    {
        ...
        $title = 'Daftar Pinjaman Barang IIS';
        $vue = "<iis-borrow-page :title='".json_encode($title)."' :constant='".json_encode($constant)."' />";
    ...
    public function read(Request $request)
    {
        if ($request->req === 'table') {
            ...
            $canViewAll = $user->can('iis.borrow.view_all');
            ...
            $pdf = \PDF::loadView(
                'print.iis.serah_terima_peminjaman',
        ...
        if ($request->req === 'write') {
            ...
            $request->validate(
                [
                    ...
                    'borrowed_due_at' => 'required',
                ],
                [
                    'asset_type.required' => 'Jenis barang wajib dipilih.',
                    'asset_type.in' => 'Jenis barang tidak valid.',
                    'to_type.required' => 'Tipe tujuan pinjaman wajib dipilih.',
                    'to_type.in' => 'Tipe tujuan pinjaman tidak valid.',
                    ...
                    'items.required' => 'Barang pinjaman wajib dipilih.',
                    'items.array' => 'Format barang pinjaman tidak valid.',
                    'items.min' => 'Minimal harus memilih 1 barang.',
                    'items.*.integer' => 'Item barang tidak valid.',
                    'borrowed_at.required' => 'Tanggal pinjaman wajib dipilih.',
                    'borrowed_due_at.required' => 'Tanggal Tenggat pinjaman wajib dipilih.',
                ]
            );
                ...
                return DB::transaction(function () use ($request) {
                    ...
                    if ($request->id) {
                        ...
                        $movement = IisMovement::create([
                            ...
                            'borrowed_due_at' => $request->borrowed_due_at,
                        ]);
                    }
    ...
    private function generateCode(): string
    {
        ...
        return sprintf('BRW-IIS-%s-%04d', $year, $number);
    }
