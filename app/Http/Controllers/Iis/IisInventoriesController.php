...
class IisInventoriesController extends AuthController
{
    public function __invoke(Request $request)
    {
        if ($request->req === 'open') {
            ...
            $title = 'Daftar Inventaris IIS';
            $vue = "<iis-inventory-page :title='".json_encode($title)."' :constant='".json_encode($constant)."' />";
        }
    ...
    {
        if ($request->req == 'table' || $request->req === 'excel' || $request->req === 'pdf') {

            $query = IisInventory::with(['bUser', 'lastPrintBy', 'unit', 'room', 'item', 'building'])
            ...
            if ($request->req === 'pdf') {
                ...
                $pdf = Pdf::setOptions([
                    ...
                    'chroot' => public_path(),
                ])->loadView('pdfs.iis.iis_inventories_data', compact('inventories'))
                    ->setPaper('a4', 'landscape');

                // stream ke browser
                return $pdf->stream('inventories_data.pdf');
            }
